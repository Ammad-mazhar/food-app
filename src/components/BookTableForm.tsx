...
export default function BookTableForm({
  ...
  // Prefill from the signed-in profile once, adjusted during render so it
  // lands before first paint and never overwrites later typing.
  ...
  /**
   * Sends the booking to the server, which holds the table.
   *
   * A 409 here is the real case worth handling well: the database refuses a
   * double-booking, so two people racing for the same slot get a clear
   * "pick another time" rather than both being told yes.
   */
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "We couldn't take that booking. Please try again.");
        setSubmitting(false);
        return;
      }
      ...
      setError("Network problem — your booking was not made.");
      setSubmitting(false);
    }
  }
